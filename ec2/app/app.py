#!/usr/bin/env python3
"""
Vault -> AWS credential demo on a virtual machine (EC2).

The point of this demo is what is MISSING. There is no Vault address, no Vault
token, no login, no Vault API call and no Vault library anywhere in this file.
Vault Agent, running as a service on this instance, does all of that and writes
credentials to files.

This application only:
  1. reads two files, and
  2. calls AWS with whatever it finds in them.

This is byte-for-byte the same application shape as the Kubernetes version. Only
the file paths differ - the platform changed, the application did not.
...
# neither is a secret. Vault Agent decides what lands in them.
STATIC_FILE = os.environ.get("STATIC_CRED_FILE", "/etc/app/secrets/aws-static.json")
DYNAMIC_FILE = os.environ.get("DYNAMIC_CRED_FILE", "/etc/app/secrets/aws-dynamic.json")
...
STATE = {
    ...
    "config": {
        ...
        "instance": os.environ.get("INSTANCE_ID", "ec2"),
    },
}
LOCK = threading.Lock()
SEQ = [0]
...
def read_cred_file(path):
    ...
    try:
        ...
        return {"exists": False, "error": "not present yet - Vault Agent has not rendered it"}
...
def refresh(kind, path, force=False):
    ...
    if changed and old_key and old_key != d.get("access_key"):
        log("agent → file", f"re-rendered {os.path.basename(path)}",
...
def poller():
    log("app", "started", "watching the two files Vault Agent writes", "ok")
    while True:
...
<title>Vault Agent on EC2 - Demo</title>
<style>
...
.a-agent{background:#f5eefb;color:#6b3fa0;border-color:#ddcbf0}
...
<div class="wrap">
  ...
  <div class="hero">
    ...
    <p><strong>Nothing.</strong> There is no Vault address, no token, no login and no Vault
       library in this app. Vault Agent, a service on this instance, authenticates on the machine's behalf and writes
    ...
    <div class="files">
      <div class="file" id="fstatic">
        ...
        <div class="blob" id="fstatic-body">waiting for Vault Agent…</div>
      </div>
      <div class="file" id="fdynamic">
        ...
        <div class="blob" id="fdynamic-body">waiting for Vault Agent…</div>
      </div>
    </div>
    <p class="note">Both files are owned by Vault Agent. When Vault rotates the Phase 1 key the
       agent rewrites the file; when the Phase 2 lease nears expiry it fetches a new credential
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <p class="why">No standing credential. Vault assumes an IAM role on demand and the
         result expires by itself. Vault Agent renews it before it lapses.</p>
      <div class="lbl">access key id in the file right now</div>
...
async function load(){
  ...
  $('cfg').textContent = `instance=${c.instance}  ·  region=${c.region}`;

  const ready = (F.static&&F.static.exists) && (F.dynamic&&F.dynamic.exists);
  const p=$('pill');
  p.className='pill '+(ready?'ok':'err');
  p.textContent = ready ? 'both credential files present · no Vault code on this host'
                        : 'waiting for Vault Agent to render files';
  ...
  $('log').innerHTML = (S.log||[]).slice(0,45).map(e=>{
    const cls = e.actor.startsWith('app → aws') ? 'a-appaws' : (e.actor.startsWith('agent')?'a-agent':'a-app');
